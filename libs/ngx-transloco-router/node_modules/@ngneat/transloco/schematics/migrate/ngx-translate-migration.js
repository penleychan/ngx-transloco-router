...
function run(path) {
    ...
    ].map(regex => (Object.assign({}, noSpecFiles, { from: regex, to: `import { TranslocoService } from '@ngneat/transloco';` })));
    const constructorInjection = Object.assign({}, noSpecFiles, { from: /(?:private|protected|public)\s+(.*?)\s*:\s*(?:TranslateService|TranslatePipe\s*(?:,|\)))/g, to: match => match.replace(/TranslateService|TranslatePipe/g, 'TranslocoService') });
    const serviceUsage = Object.assign({}, noSpecFiles, { from: /(?=([^]+(?:private|protected|public)\s+([^,:()]+)\s*:\s*(?:TranslocoService\s*(?:,|\)))))\1[^]*/gm, to: (match, _, serviceName) => {
